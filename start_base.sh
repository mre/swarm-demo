...
curl -X PUT -d "{\"ID\": \"$INFLUXDB_ID\", \"Name\": \"api.influxdb\", \"Address\": \"$INFLUXDB_IP\", \"Port\": 8086, \"Check\": { \"HTTP\": \"http://$INFLUXDB_IP:8086/query?q=SHOW%20DATABASES\", \"Interval\": \"2s\"  }}" http://$CONSUL_IP:8500/v1/agent/service/register
curl -X PUT -d "{\"ID\": \"$INFLUXDB_ID\", \"Name\": \"graphite.influxdb\", \"Address\": \"$INFLUXDB_IP\", \"Port\": 2003, \"Check\": { \"HTTP\": \"http://$INFLUXDB_IP:8086/query?q=SHOW%20DATABASES\", \"Interval\": \"2s\"  }}" http://$CONSUL_IP:8500/v1/agent/service/register
